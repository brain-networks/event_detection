...
(
    ...
    etspeaks_orig_sur,
    mu_orig_sur,
) = ev.event_detection(DATA_file, opj(TEMP, "atlas.nii.gz"), opj(TEMP, "surrogate_"))
...
ets_AUC, rss_AUC, rssr_AUC, idxpeak_AUC, etspeaks_AUC, mu_AUC = ev.event_detection(
...
axs[0].plot(
    range(rss_orig_sur.shape[0]), rss_orig_sur, "k", "linewidth", 2, label="orig_sur"
)
...
plt.plot(
    range(rss_orig_norm.shape[0]), rss_orig_norm, "k", "linewidth", 3, label="orig_sur"
)
...
plt.plot(
    range(rss_fitt_norm.shape[0]), rss_fitt_norm, "y", "linewidth", 3, label="fitted"
)
plt.legend()
